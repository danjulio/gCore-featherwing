...
// LVGL integration
//
bool tp_read(lv_indev_drv_t * indev_driver, lv_indev_data_t *data)
{
...
//    Serial.printf("%d %d %d\n", x, y, c);
    ...
    // Flush buffer when we haven't detected a touch - not sure why, but this seems
    // necessary to prevent a case where we read stale data
    if (!_ts_buffer_empty()) {
      _ts_write_8bit_reg(STMPE_FIFO_STA, STMPE_FIFO_STA_RESET);
      _ts_write_8bit_reg(STMPE_FIFO_STA, 0); // unreset
    }
